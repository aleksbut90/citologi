...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        // Настройка заголовков (разрешаем iframe для H2 console если нужно, или просто отключаем защиту)
        http.headers(headers -> headers.frameOptions(frame -> frame.disable()));

        // Отключаем CSRF (Vaadin обрабатывает это сам через токены)
        http.csrf(csrf -> csrf.disable());

        // Настройка кэша запросов для редиректа после логина
